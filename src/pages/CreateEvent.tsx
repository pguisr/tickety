...
import { DateTimeInput, DurationPreview } from '@/components/ui/datetime-input';
...
const CreateEvent: React.FC = () => {
  ...
  const [initialFormData] = useState<EventFormData>({
    title: '',
    description: '',
    startsAt: '',
    endsAt: '',
  ...
  // Aplica sugestão de URL
  const applyUrlSuggestion = (suggestion: string) => {
    setFormData(prev => ({ ...prev, url: suggestion }));
    setUrlError(''); // Limpa o erro
    setUrlSuggestions([]); // Limpa as sugestões
  };
  ...
  const validateUrl = async (url: string): Promise<{ valid: boolean; error?: string; suggestions?: string[] }> => {
    if (!url) {
      return { valid: true };
    }
  ...
  // Gera sugestões de URL
  const generateUrlSuggestions = (baseUrl: string): string[] => {
    const suggestions = [];
  ...
  const handleSave = async () => {
    ...
    setIsLoading(true);
    setUrlError(''); // Limpa erros anteriores
    setUrlSuggestions([]); // Limpa sugestões anteriores
    ...
    try {
      ...
      if (!formData.location?.trim()) {
        sonnerToast.error('O local do evento é obrigatório.');
        return;
      }
      
      if (!formData.address?.trim()) {
        sonnerToast.error('O endereço do evento é obrigatório.');
        return;
      }
      ...
      // Validação de URL melhorada
      const urlValidation = await validateUrl(formData.url);
      ...
      // Gera URL automaticamente se não for fornecida
      let eventUrl = formData.url;
      if (!eventUrl) {
        eventUrl = formData.title
          .toLowerCase()
          .replace(/[^a-z0-9\s-]/g, '')
          .replace(/\s+/g, '-')
          .replace(/-+/g, '-')
      ...
      // Cria o evento usando o repositório Supabase
      ...
      if (formData.batches?.length > 0) {
        await batchesRepository.updateBatches(newEvent.id, formData.batches);
      }
      
      sonnerToast.success('Evento criado com sucesso.');
      ...
      // Verifica se é erro de URL duplicada
      if (error.message && error.message.includes('URL') && error.message.includes('já está em uso')) {
        setUrlError(error.message);
        // Gera sugestões de URLs alternativas
        const suggestions = generateUrlSuggestions(formData.url);
        setUrlSuggestions(suggestions);
      } else {
        sonnerToast.error(error.message || 'Não foi possível criar o evento. Tente novamente.');
      }
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen bg-tickety-black">
      ...
      <PageTransition>
        ...
        <div className="mb-6 lg:mb-2">
                  <h1 className="text-2xl lg:text-3xl font-semibold text-white mb-1 px-4">Criar Novo Evento</h1>
        <p className="text-xs lg:text-sm text-gray-400 px-4">Configure seu evento do zero</p>
        </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center">
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          // Limpa erro e sugestões quando usuário digita
                          if (urlError) {
                            setUrlError('');
                            setUrlSuggestions([]);
                          }
                    ...
                    {/* Erro de URL duplicada */}
                    {urlError && (
                      <div className="flex items-start gap-2 mt-2">
                        <div className="w-4 h-4 bg-red-500 rounded-full flex items-center justify-center mt-0.5 flex-shrink-0">
